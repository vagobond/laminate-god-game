-- Drop and recreate the function with SECURITY DEFINER to allow unauthenticated viewing
DROP FUNCTION IF EXISTS public.get_visible_profile(uuid, uuid);

CREATE OR REPLACE FUNCTION public.get_visible_profile(
  viewer_id uuid,
  profile_id uuid
)
RETURNS TABLE (
...
BEGIN
  ...
  SELECT 
    ...
    -- Birthday day/month: based on user's visibility setting
    ...
    -- Birthday year: based on user's year visibility setting
    CASE WHEN v_is_self OR (
