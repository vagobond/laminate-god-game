-- Worklist #4: river query scalability.
-- Old get_river_entries called can_view_xcrol_entry() (plpgsql) once per row —
-- O(all entries) with no index pruning — and paginated with OFFSET. This
-- rewrite inlines the exact same visibility rules as SQL predicates so the
-- planner can run a top-N scan over a (created_at, id) index and stop at
-- p_limit visible rows, and adds keyset pagination (p_before_ts, p_before_id).
-- p_offset is kept as a fallback so already-deployed frontends keep working.

-- Index for newest-first top-N scans (the existing feed index is on
-- entry_date, created_at and cannot serve created_at ordering).
create index if not exists idx_xcrol_entries_created_at_id
  on public.xcrol_entries (created_at desc, id desc);

-- Signature changes (two new params), so the old function must go first —
-- keeping both would make PostgREST RPC resolution ambiguous.
drop function if exists public.get_river_entries(uuid, integer, integer, text);
