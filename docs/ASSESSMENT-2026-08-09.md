...
- [x] **4. River query scalability.** DONE 2026-08-10 (PR #13, migration 20260811020000): per-row `can_view_xcrol_entry()` calls replaced with identical inline SQL visibility predicates + new `(created_at desc, id desc)` index → planner does a top-N index scan that stops at p_limit visible rows; keyset pagination (`p_before_ts`/`p_before_id`, `sort_at` in return) with `p_offset` kept as rollout fallback; TheRiver.tsx switched to the cursor. Parity proven: old-vs-new EXCEPT = 0 rows both directions on prod (3 viewers × 4 filters), keyset walk = single page, and RLS suite now asserts river/ladder parity (`[5a]`) + keyset walk (`[5b]`) in CI. Live post-merge: EXPLAIN reads exactly 20 rows for a 20-row page. Original scope: `get_river_entries` called `can_view_xcrol_entry()` per row (O(all entries), no index pruning) + OFFSET pagination — first scaling wall at low tens of thousands of entries.
- [x] **5. Connection-degree BFS.** DONE 2026-08-10 (doc fix per this item's own recommendation): README + getting-started copy now say 3 degrees (the in-app badge calls `get_connection_degree` with the default max_depth 3); the OAuth API path (`oauth-connection-degree`) explicitly passes max_depth 6 and is unchanged — README notes the distinction. BFS scaling wall itself deliberately deferred: query-time recursive CTE is correct at 83 users; revisit at a few thousand densely-connected users. Original scope: `get_connection_degree_fast` = query-time recursive CTE over `friendship_pairs`, max_depth 3. Cheap now; wall #2 at a few thousand densely-connected users. Also: README said "6 degrees," function defaults to 3.
- [x] **6. Uptime monitoring.** DONE 2026-08-10 (PR #15): `.github/workflows/uptime.yml` curls xcrol.com and the health edge function every 30 min; a failed run emails CD via GitHub's workflow-failure notification — no third-party accounts, no secrets. Caveat (documented in RUNBOOK): GitHub disables cron workflows after 60 days of repo inactivity; any commit re-enables. Original scope: nothing alerts if xcrol.com or the health function goes down.
- [x] **7. Arm the dead-man's switch.** DONE 2026-08-15: armed via secrets (DEADMAN_ENABLED=1, TRUSTEE_EMAIL=fawndamitio@gmail.com, DEADMAN_DAYS=90). 2026-08-18 (PR #31) hardened it: activity signal now = greatest of admin-dashboard heartbeat / last sign-in / session refresh / latest admin post (was /admin-load only), and the alert sends from the Resend-verified noreply@invites.xcrol.com and checks the send response (was trustee@xcrol.com, which silently 403'd). Weekly heartbeat-check cron confirmed running.
...
- [x] **9. 140 vs 240 characters.** RESOLVED 2026-08-10: CD confirmed 240 is canonical ("I made a gaffe when I said 140"). Audit found the codebase already fully consistent: DB CHECK constraints (`xcrol_entries`, `brook_posts`) at 240, UI composers at maxLength 240, README at 240, zero stray "140" in any copy. No changes needed.
